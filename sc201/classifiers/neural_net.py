...
class TwoLayerNet(object):
    ...
    def cost(self, X, y=None):
        ...
        #K A F(scores)
        K = np.dot(X, W1) + b1.reshape((b1.shape[0], 1)).T # (N, H)
        A = np.maximum(0, K)  # (N, H)
        scores = np.dot(A, W2) + b2.reshape((b2.shape[0], 1)).T # (N, C)
        ...
        H = np.exp(scores_y)/np.sum(np.exp(scores), axis=1)
        cost = (1/N)*np.sum(-np.log(H))
        ...
        sigma[range(N), y] -= 1
        grads_classScores = sigma / N
        grads['b2'] = np.sum(grads_classScores, axis=0) #(, C)
        grads['W2'] = A.T@grads_classScores # (H, C)
        grads['b1'] = np.sum((grads_classScores@(W2.T))*np.where(K>0,1,0),axis=0) #(, H)  
        grads['W1'] = X.T@((grads_classScores)@(W2.T)*np.where(K>0,1,0)) #(D, H)
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            #########################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            minibatch = np.random.randint(num_train, size = batch_size)
            X_batch = X[minibatch]
            y_batch = y[minibatch]
            # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ...
    def predict(self, X):
        ...
        ###########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        K = np.dot(X, self.params['W1']) + self.params['b1'].T # (N, H)
        A = np.maximum(0, K)  # (N, H)
        scores = np.dot(A, self.params['W2']) + self.params['b2'].T # (N, C)
